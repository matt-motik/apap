...
[x] Шаг 3: Tray-notice — `src/tray.rs` + `src/app/mod.rs`: `TrayState.notice`, override `tool_tip()`, поле `bp_notice_until`, `set_bp_notice()` при включении BP в settings-save, подстановка notice в `tray_state()`. Проверка: `cargo check` + `cargo test` зелёные
...
- **Следующий ход:** Финальный аудит `cargo check + clippy + test`, ROADMAP → done, консервация `_STATE_.md`, `git diff --stat`
