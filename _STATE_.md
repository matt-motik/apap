...
- **Следующий ход:** `ui/settings.slint` + `ui/app.slint`: добавить in-properties `theme-list-model <[string]>`, `theme-current <string>`, `theme-name <string>`, `theme-description <string>`, `theme-save-enabled <bool>` и callback `settings-theme-selected(string)`; заменить ComboBox (строки 230-234) на `model: root.theme-list-model` / `current-value: root.theme-current` + два `Text` метаданных под ним + `enabled: root.theme-save-enabled` у кнопки Save; проброс callback в app.slint (строка ~480). Проверка: cargo check.
