...
- **Следующий ход:** Убрать `FULLTRACK_CACHE_LEN` в src/app/mod.rs, инициализировать `CLruCache` через `ft::fulltrack_cache_max_entries(settings.visualization.viz_max_ram_mb)`, затем `cargo check`
