...
## Итерационный трекер
[ ] Шаг 1: `fulltrack.rs`: статистика дискового кэша — `disk_cache_size()` + ядро `disk_cache_size_in(dir) -> u64` (сумма всех файлов viz-каталога, отсутствующий каталог → 0). Проверка: `cargo test disk_cache_size` зелёный
[ ] Шаг 2: `fulltrack.rs`: человекочитаемый формат `fmt_cache_bytes(u64) -> String` (B/KiB/MiB, 2 знака) + юнит-тест. Проверка: `cargo test fmt_cache_bytes` зелёный
...
- **Следующий ход:** Реализовать `disk_cache_size()`/`disk_cache_size_in` в `src/audio/fulltrack.rs` рядом с `evict_disk_cache_in`, написать юнит-тест по образцу `evict_disk_cache_*` (временный каталог + `dir_size`), запустить `cargo test`.
