...
[ ] Шаг 2: Расширить `DeviceInfo` полями `category`, `supported_rates`, `supported_formats`, `exclusive_capable`; заполнение в `CpalHost::devices` и в мок-хелпере `mock_device_id`. Проверка: `cargo check` без ошибок.
...
- **Следующий ход:** Расширить `DeviceInfo` полями `category`, `supported_rates`, `supported_formats`, `exclusive_capable`; заполнение в `CpalHost::devices` и мок-хелпере `mock_device_id`; затем `cargo check`
