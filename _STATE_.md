...
- **Задача 4.10 (шаг 4, быстрый таймер рефлоу):**
  ...
  - Решение: отдельный быстрый таймер только для рефлоу — `reflow()` (`MusicApp`, бывший блок тика) на 16 мс (~60 Гц); основной `tick()` остался на 100 мс (COL_SAVE-debounce драга колонки не тронут). `REF_INTERVAL_MS = 16` в `main.rs`.
- Верификация (шаг 4): build ok; тесты 55 lib + 6 bin зелёные; clippy — warning в `reflow()` (лишний `as f32`) убран.
...
- [x] **Быстрый таймер рефлоу** (`reflow()` на 16 мс в `main.rs`): колонки догоняют мышь при росте окна, независимо от 100мс `tick()`.
- [x] build / clippy / test; ROADMAP + _STATE_ обновлены.
- [ ] Ручная проверка на живой системе (нужен дисплей): **плавность роста окна при быстром ресайзе**, рост/сжатие окна (GNOME FHD), драг разделителя колонки (задержка после отпускания), KDE 4K «упирается в границу».
- [ ] (Если KDE 4K-кейс не решён) — доп. диагностика ограничения размера окна.

## Следующий ход

- Закоммитить изменения 4.10 (код + `_STATE_.md` + `ROADMAP.md`).
- Попросить пользователя проверить на живой системе: **рост окна при быстром ресайзе стал плавным** (16мс рефлоу)? Если да — вопрос CPU/стабильности (60 Гц во время ресайза). Если нет — варианты: увеличить `REF_INTERVAL_MS`, либо ресайз-пересчёт напрямую по событию окна (если Slint даёт геометрию в реальном времени).

## Изменяемые файлы (текущий шаг)

- `src/main.rs` (`REF_INTERVAL_MS=16`, второй `Timer` для `MusicApp::reflow`)
- `src/app/mod.rs` (method `reflow()` — вынесен из `tick()`; удалены `view_w_settle_ticks`, `VIEW_W_SETTLE_TICKS`)
- `src/app/ui_manager.rs` (`sync_playlist_to_ui` → `set_vec`; `update_column_widths` → точечные widths; `apply_window_geometry` → fallback-геометрия)
- `ui/app.slint` (убран `preferred-width/height`)
...
- **KDE 4K «упирается в границу»**: preferred-width фикс не помог (пользователь перепроверил). Гипотеза: стабилизационный дебаунс давал «заморозку»/«дебаунс». Заменён на **синхронный рефлоу** — окно и колонки всегда в связке. Нужна живая проверка; если KDE-кейс останется — копать slint-window resize limits / geometry scale.
- **Риск синхронного рефлоу**: рефлоу на 16 мс во время ресайза может грузить CPU (60 Гц пересчёт `set_row_data`). Если окажется дорого/дёргано — увеличить `REF_INTERVAL_MS` (33/50 мс) или пересчёт по событию ресайза окна. Стабильный debounce записи на диск и коррекции мин/макс (`COL_SAVE_DEBOUNCE_TICKS`) остался на 100мс `tick()` — не конфликтует с рефлоу.