...
[ ] Шаг 2: Удаление toggle-хендлера — `ui/app.slint` (корневой `callback toggle-bit-perfect()` :233) + `src/app/mod.rs` (`ui.on_toggle_bit_perfect` :540-561). Проверка: `cargo check` зелёный
...
- **Следующий ход:** Удалить корневой `callback toggle-bit-perfect()` из `app.slint` и хендлер `ui.on_toggle_bit_perfect` из `src/app/mod.rs`
