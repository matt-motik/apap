...
- Верификация (шаг 3): build ok; тесты 55 lib + 6 bin зелёные.
...
- [ ] Ручная проверка на живой системе (нужен дисплей): рост/сжатие окна мышью (GNOME FHD), драг разделителя колонки (задержка после отпускания), KDE 4K «упирается в границу».
- [ ] (Если KDE 4K-кейс не решён) — доп. диагностика ограничения размера окна.

## Следующий ход

- Закоммитить изменения 4.10 (код + `_STATE_.md` + `ROADMAP.md`).
- Попросить пользователя проверить на живой системе: **ресайз окна стал плавным/синхронным с колонками?** Ключевой признак: таблица больше не «заморожена» во время ресайза, колонки следуют за границей окна.

## Изменяемые файлы (текущий шаг)

- `src/app/mod.rs` (поле `playlist_cols`, инициализация в `new()`; тик-блок рефлоу → синхронный; удалены `view_w_settle_ticks`, `VIEW_W_SETTLE_TICKS`)
...
- **Риск синхронного рефлоу**: если `update_column_widths` при большом списке окажется дорогим (relayout таблицы на каждый тик при ресайзе) — вернуться к умеренному троттлингу (например пересчёт каждый 2-й тик, но не «заморозка» ~300мс), либо точечный рефлоу только части колонок.