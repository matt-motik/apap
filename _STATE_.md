...
- Верификация (шаг 1): build ok; новых clippy-warning'ов в `app/*` нет; тесты 55 lib + 6 bin зелёные.
- Верификация (шаг 2, стабилизационный рефлоу): build ok, тесты 55 lib + 6 bin зелёные.
...
- [x] Убран `preferred-width/height` окна + fallback-геометрия (первый запуск) — фикс медленного горизонтального роста окна.
