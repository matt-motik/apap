...
- Попросить пользователя проверить на живой системе: **сужение/расширение колонки больше НЕ двигает окно**, рост окна быстрый и свободный, колонки догоняют таблицу после релиза (~64мс). Это ключевой признак разрыва связи окно↔Σ колонок.

## Изменяемые файлы (текущий шаг)

- `ui/playlist.slint` (`max-width: 10000px` у `StandardTableView` — разрыв связи окна с Σ колонок)
- `src/app/mod.rs` (method `reflow()` — рефлоу только после стабилизации ~64мс: заморозка во время ресайза + один пересчёт; поля `last_view_width`, `view_w_stable_ticks`; константа `REFLOW_SETTLE_TICKS=4`)
- `src/main.rs` (таймер `reflow()` на 16 мс — теперь только детект стабилизации, без живой перезаписи)
