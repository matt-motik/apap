...
- **Следующий ход:** В `fulltrack.rs` добавить тесты `.3a` (проверка `cache_key`/`cache_key_spectrogram` чувствительны к `dsd_params`) и `.3b` (тест `cache_meta_valid_in` на TTL по mtime sidecar).
