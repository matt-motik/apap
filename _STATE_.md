...
[ ] Шаг 2: Чистая функция `fulltrack_cache_max_entries(max_ram_mb) -> NonZeroUsize` в `audio/fulltrack.rs` (формула `mb / avg_rgba_mb`, avg = 2000×512×4 B). Проверка: `cargo check` без ошибок
[ ] Шаг 3: Убрать константу `FULLTRACK_CACHE_LEN`, инициализация `CLruCache` от `ft::fulltrack_cache_max_entries(viz_max_ram_mb)` в `MusicApp::new`. Проверка: `cargo check` без ошибок
...
- **Следующий ход:** Добавить `fulltrack_cache_max_entries` в src/audio/fulltrack.rs (const `AVG_FULLTRACK_RGBA_BYTES` + безопасная формула), затем `cargo check`
