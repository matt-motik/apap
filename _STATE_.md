...
[x] Шаг 2: `src/audio/player.rs` — поле `ring_buffer_ms`, `set_ring_buffer_ms`, `ring_capacity(ms, buffer_frames)` (нижняя граница ≥ 2× cpal-буфера при Fixed, иначе 4096), применение в `start_engine`; юнит-тест ms→samples. Проверка: `cargo test audio::player` зелёный.
[x] Шаг 3: `src/app/mod.rs` — проброс `settings.audio.ring_buffer_ms` в `player.set_ring_buffer_ms` при инициализации. Проверка: `cargo check` + полный `cargo test` + `cargo clippy` без новых предупреждений.

- **Текущий шаг (current_step):** Завершение (Шаг 5 AGENTS)
- **Следующий ход:** Обновить ROADMAP.md (A2.0-5.2 → ✅), законсервировать `_STATE_.md`.
