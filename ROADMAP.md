...
**Статус:** 🔶 частично — аудио-колбэки уже защищены через `try_lock()` (строки 343–405); в основных методах `Player` на строке 180/204/224/248/264/327 остались `self.core.lock().unwrap()`.
...
|---|--------|-----------|---------|----------------|----------------|
| 1.1 | Безопасная обработка lock() в player.rs | 🔴 Критический | 🔶 частично | 2ч | Низкий |
