...
- **Убран `preferred-width/height` окна** (`ui/app.slint`) + fallback-дефолт `logical 1200x760` в `apply_window_geometry` (первый запуск). **НЕ помог** — пользователь перепроверил свежую сборку: таблица по-прежнему «заморожена» на момент клика.
- **Синхронный рефлоу по ширине (промежуточный — откачен):** стабилизационный дебаунс (`VIEW_W_SETTLE_TICKS=3`) убран; колонки пересчитываются при каждом значимом изменении ширины (>1px) лёгким `update_column_widths()` (set_row_data). Помог (стало быстрее), но рост всё равно отставал.
- **Быстрый таймер рефлоу (промежуточный — откачен):** отдельный 16мс таймер (`reflow()`). **Стало хуже**: и уменьшение, и увеличение отстают; сужение колонки уменьшает размер окна.
- **КОРНЕВОЙ ФИКС 4.10.** Найден в исходниках Slint (`material/tableview.slint:194,237` + `winitwindowadapter.rs:1407-1428,1690-1721`): каждая колонка жёстко зажата `min-width = preferred-width = max-width = column.width` → max-ширина корневого layout = Σ column.width → winit ставит окну `set_max_inner_size` = Σ колонок и принудительно подгоняет размер окна под [min,max]. Поэтому: окно упирается в Σ колонок при росте («стена»), а сужение колонки драгом уменьшает Σ → окно **само сжимается**.
  - **Фикс 1:** `max-width: 10000px` у `StandardTableView` в `ui/playlist.slint` — разрыв связи размера окна с Σ column.width. Окно ресайзится свободно и никогда не сжимается колонкой.
  - **Фикс 2 (решение пользователя «пересчёт только на релиз»):** `reflow()` теперь замораживает колонки во время ресайза и пересчитывает их один раз после стабилизации ширины (`REFLOW_SETTLE_TICKS=4`, ~64 мс ≈ отпуск мыши). Живой пересчёт на каждый тик убран.
...
- Уменьшить шрифт названий колонок — **отложено** (требует кастомизации material StandardTableView либо глобального font; решение — за пользователем).
