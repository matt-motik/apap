...
- **Убран `preferred-width/height` окна** (`ui/app.slint`) + fallback-дефолт `logical 1200x760` в `apply_window_geometry` (первый запуск). Стабилизационный рефлоу сам по себе не убрал медленный рост (пользователь: растёт быстро «до размера таблицы», сверх — ползёт; одинаково с любым списком). Причина: `preferred-width: 1200px` «притягивает» окно и тормозит горизонтальный рост сверх него. Без preferred окно растёт свободно и быстро.

**Осталось (живая проверка, нужен дисплей):**
- Ресайз окна мышью на Ubuntu GNOME FHD — проверить, что **горизонтальный рост ускорился** после удаления preferred-width (окно теперь не «притягивается» к 1200px), колонки «дощёлкивают» через ~300 мс после остановки.
