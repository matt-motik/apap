...
- **Синхронный рефлоу по ширине (итоговый фикс 4.10):** стабилизационный дебаунс (`VIEW_W_SETTLE_TICKS=3`) **убран**. Диагноз: «заморозка таблицы» и есть дебаунс — пока `visible-width` не стабильна ~300 мс, колонки не трогаются → при непрерывном ресайзе таблица выглядит мёртвой, окно растягивается за фиксированную (сумма `column.width`) таблицу и упирается. Причина не в preferred-width, а в несинхронности колонок с окном. Теперь колонки пересчитываются при каждом значимом изменении ширины (>1px) лёгким `update_column_widths()` (set_row_data) — таблица всегда = ширине окна. Поле `view_w_settle_ticks` и константа `VIEW_W_SETTLE_TICKS` удалены.
- **Отдельный быстрый таймер рефлоу** (`REF_INTERVAL_MS=16`, ~60 Гц в `main.rs`): `MusicApp::reflow()` вынесен из `tick()`. Синхронный рефлоу в общем 100мс тике устранил «заморозку», но рост всё ещё отставал (рефлоу ≤10 раз/с). Самостоятельный 16мс таймер читает `visible-width` и обновляет `.width` колонок почти на каждой частоте движения мыши → таблица следует за ростом окна. Основной `tick()` (100 мс) не тронут: COL_SAVE-debounce драга колонки работает как раньше.

**Осталось (живая проверка, нужен дисплей):**
- Ресайз окна мышью на Ubuntu GNOME FHD — проверить, что **рост окна стал плавным** (16мс рефлоу), таблица следует за границей без «дебаунса»/отставания, CPU терпимый.
