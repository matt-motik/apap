...
**Подтверждённые оптимизации (accel):**
5. **LUT палитр** (`src/audio/spectrogram.rs:170-174`): `palettes::parse_hex(hex)` парсит 1536-байтную строку на каждый пиксель (~1M раз/трек). Решение: предвычислить LUT в `Spectrogram::new`, O(1)-lookup.
6. **RowPlan** (`compute_column`): `freq_for_row`/`cic_compensation_db`/`level_of` (powf/log10/sin) считаются на каждый пиксель, хотя зависят только от строки. Решение: предвычислить на строку (bin_start, mag_scale, boost_db).
7. **Аллокации в цикле** (`compute_column`): `wbuf`/`mags` создаются `Vec` на каждую колонку×канал. Решение: переиспользовать буферы в полях структуры.
8. **Профиль релиза**: `lto = true` → `lto = "fat"`, `codegen-units = 1`, `panic = "abort"`; `.cargo/config.toml` → `target-cpu=native`.
...
5. Спектрограмма (полнотрековая): FFT по треку, палитры, адаптивный hop, CIC-компенсация — ✅ сделано в `e082148`.
