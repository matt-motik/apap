...
Интерактивный режим (задачи от пользователя в чате)

    Задача, поставленная пользователем в интерактивном режиме — полноценная задача.
    Перед выполнением:
        Добавить пункт в ROADMAP.md (этап, приоритет, краткое описание).
        Завести/обновить _STATE_.md (активная задача, шаги, изменяемые файлы).
    После выполнения:
        Пометить пункт в ROADMAP.md как сделанный (✅ + commit).
        Привести _STATE_.md к done.
    Это гарантирует, что задача и нюансы реализации не потеряются при нештатном завершении сессии (обрыв токенов, смена машины, крах).
