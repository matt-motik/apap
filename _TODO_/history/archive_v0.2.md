# Архив реализации: Этапы 1–5, шаги 6.1–6.7 (v0.2)

> Этот файл содержит полную историю реализации завершённых этапов.
> Все коммиты влиты в `main`. Релиз `v0.2` обновлён (тег на `a01c1bf`).
> Файл создан при чистке ROADMAP.md 2026-09-13.
...
**Статус:** ✅ сделано — eager-`settings.save()` убраны из volume (GUI, tray wheel), shuffle (заодно убран дубль `rebuild_shuffle`), repeat, sort prefs, save*column_widths. Сохранение `Settings` теперь происходит при выходе: оба выхода (tray Quit и close окна без minimize) вызывают `save_window_geometry()` → `settings.save()` (флашит всё). `on_settings_save` (Save в диалоге) — diff-apply: после `settings.settings = draft` переживает поля, не управляемые диалогом (volume/muted/last_dir/repeat/shuffle/sorted_col/sort_desc/win*\*), чтобы не затереть live-изменения свежим значением из клона-драфта. Диалог настроек остаётся draft+cancel (закрытие без изменений).
...
диска на лету не чем — внешнего редактора/паттерна watch нет. При изменении настроек
...
## Future-блок (прежний, до реструктуризации)
...
**Статус:** ✅ 6.1 ✅, 6.2 ✅, 6.3 ✅, 6.4 ✅, 6.5 ✅, 6.6 ✅, 6.7 ✅ — каркас, анализатор, DSD/ресемплеры, осциллограмма, спектрограмма, переключение/UI, оптимизации+баги. Все коммиты влиты в `main`.
...
### 6.2. Анализатор спектра (мгновенный) ✅
...
### 6.3. Доработка DsdDecoder + Ресемплеры ✅
...
### 6.4. Осциллограмма (полнотрековая) ✅

**Коммит:** `6dc2dc5`/`ddeb176` (совместно с баг-фиксом окна).
...
- **Верификация:** 91 lib + 6 bin ✅, clippy 0 новых, build --release ок.

**Архитектура:** FullTrackWorker (полнотрековые, отдельный поток, та же конвертация, что у плеера — WYSIWYG) + LiveWorker (мгновенный, tap из cpal callback в ring buffer, FFT в отдельном потоке); настройки читаются атомарно на границе кадра (для LiveWorker — `RwLock<Arc<VisualizerConfig>>`, без arc_swap); UI-поток не считает FFT.

**Баг-фикс: окно зажималось / layout визуализатора** — winit брал размер окна из WM-хинтов и резал до max ≈ 657 px (сумма max-лимитов фиксированных колонок top_panel); `horizontal-stretch: 1` у колонки визуализатора упиралась в её собственный расчётный max (~119 px). Фикс обоих — приём «щедрый max-width»: `max-width: 100000px` на корне `AppWindow` (`ui/app.slint`) и на колонке визуализатора (`top_panel.slint`), col 3 обратно `width: 40px`. Осциллограмма: `image-fit: fill` вместо `contain` (картинка 16:1 в плейсхолдере ~6:1). Проверено xprop (user 1920×1008 / max 100000) и пользователем. Коммиты `ddeb176` + `6dc2dc5`.

---

### 6.5. Спектрограмма (полнотрековая) ✅
...
- **Верификация:** build + clippy (0 новых, baseline 22/9) + тесты 100 lib + 6 bin ✅; smoke на реальных файлах: 10 c sweep 280 мс; «02. Rome.flac» 96 кГц/стерео/4:30 — 23.3 с декод symphonia (FFT ~40x realtime), 4000×512 = 8 МБ ≤ 128 МБ; пик лог-чирпа 200→8000 Гц отслежен по ярчайшей строке (бины сходятся).

---

### 6.6. Переключение типа + UI настроек визуализации (ТЗ §3.2/§9) ✅
...
- **Доработка (после проверки):** по запросу пользователя и правке `ui/app.slint` кастомный `AppMenuBar` (Rectangle+кнопки) дополнен нативным `MenuBar` первым ребёнком `AppWindow` с меню «Файл»/«Визуализация» (checkable, §3.2)/«Настройки». `menu_select_viz_mode` (клик по отмеченному → Off). V-хоткей (FocusScope) сохранён.

---

### 6.7. Оптимизация визуализации + критические баги (замечания кода от внешних ИИ) ✅

**Статус:** ✅ сделано — коммиты `827d0d6` (баги), `8e29719` (спектрограмма), `b5d021c` (профиль релиза). SIMD/Rayon/Canvas — отложено «на подумать» пользователем (в бэклог).

**Источник:** `_TODO_/замечания 1`, `_TODO_/замечания 2`, `_TODO_/замечания 2.1` (ревью внешних ИИ). Верифицировано вручную против кода; `/workspace` — артефакт ревьюера, пути маппятся на этот проект.

**Подтверждённые баги (fix):**

1. **`viz_sig` не включает DSP-параметры** (`src/app/visualizer_manager.rs:62`): sig = (mode, bands, channels, bar_gap, bar_radius, gradient) → `set_cfg()` не вызывается при изменении `freq_scale`/`smoothing`/`peak_hold`/`sensitivity`/`level_scale`. Настройки живого анализатора не применяются после сохранения. Решение: расширить sig ИЛИ всегда пушить cfg (worrawer сравнивает ключи).
2. **Кэш fulltrack при `cache_in_mem=false`** (`src/app/fulltrack_manager.rs:474-489`): ключ есть в кэше, показ пропущен (cache_in_mem=false), Build не послан (contains_key==true) → вечный плейсхолдер. Решение: разделить проверку наличия и показ.
3. **`got` в `pull_frame`** (`src/audio/analyzer.rs:166-176`): локальная переменная сбрасывается при частичном чтении → сэмплы разных временных интервалов смешиваются в кадр. Решение: поле структуры.
4. **Неограниченный RAM-кэш** (`src/app/mod.rs:215`): `HashMap<String, Image>` растёт бесконечно. Решение: LRU (например `lru` crate, лимит ~20 изображений).

**Подтверждённые оптимизации (accel):** 5. **LUT палитр** (`src/audio/spectrogram.rs:170-174`): `palettes::parse_hex(hex)` парсит 1536-байтную строку на каждый пиксель (~1M раз/трек). Решение: предвычислить LUT в `Spectrogram::new`, O(1)-lookup. 6. **RowPlan** (`compute_column`): `freq_for_row`/`cic_compensation_db`/`level_of` (powf/log10/sin) считаются на каждый пиксель, хотя зависят только от строки. Решение: предвычислить на строку (bin_start, mag_scale, boost_db). 7. **Аллокации в цикле** (`compute_column`): `wbuf`/`mags` создаются `Vec` на каждую колонку×канал. Решение: переиспользовать буферы в полях структуры. 8. **Профиль релиза**: `lto = true` → `lto = "fat"`, `codegen-units = 1`, `panic = "abort"`; `.cargo/config.toml` → `target-cpu=native`.

**Отложено «на подумать» (пользователь исследует):** SIMD (`wide`) для magnitude, Rayon параллельные колонки, Canvas/Path замена Slint `for-each` → в бэклог, не трогать в 6.7.

**Верификация:** build/clippy/test (101 lib + 10 bin) ✅, release-сборка (`lto=fat`, `target-cpu=native`) — бинарь 29.4 МБ (было 34.3 МБ), спектрограмма-тесты подтверждают эквивалентность рендера.

**Релиз v0.2:** бинарь `releases/v0.2/music-player-rs` пересобран с 6.7 (29.4 МБ), тег `v0.2` перемещён на `a01c1bf`.

---

## Сводная таблица приоритетов (снимок на момент реструктуризации)

| # | Задача | Приоритет | Статус |
| --- | --- | --- | --- |
| 1.1 | Безопасная обработка lock() в player.rs | 🔴 Критический | ✅ сделано |
| 1.2 | Исправление гонок в cover.rs/playlist.rs | 🔴 Критический | ✅ сделано |
| 1.3 | Неблокирующий probe устройства | 🟡 Высокий | ✅ сделано |
| 2.1 | Разделение MusicApp на сервисы | 🟡 Высокий | ✅ сделано |
| 2.2 | Dependency Injection | 🟡 Высокий | ✅ сделано |
| 2.3 | Trait AudioHost | 🟢 Средний | ✅ сделано |
| 3.1 | Оптимизация сортировки | 🟢 Средний | ✅ сделано |
| 3.2 | Хэш-субдиректории кэша | 🟢 Средний | ✅ сделано |
| 3.3 | Асинхронная загрузка плейлиста | 🟢 Средний | ✅ сделано |
| 4.1 | Event-driven архитектура | 🔵 Низкий | ✅ сделано |
| 4.2 | ISP для AudioSource | 🔵 Низкий | ✅ сделано |
| 4.3 | Расширение тестов | 🔵 Низкий | ✅ сделано |
| 4.4 | Семантика настроек: diff-apply + save-at-exit | 🔵 Низкий | ✅ сделано |
| 4.5 | Сикбар: драг + seek при отпускании | 🔵 Низкий | ✅ сделано |
| 4.6 | Трей-громкость → UI-ползунок | 🔵 Низкий | ✅ сделано |
| 4.7 | Порядок на диске ≠ порядок просмотра | 🔵 Низкий | ✅ сделано |
| 4.8 | Баг-раунд: tray/окно wheel + настройки + ComboBox | 🔵 Низкий | ✅ сделано |
| 4.9 | Rework размеров колонок | 🔵 Низкий | ✅ сделано |
| 4.10 | Ресайз колонок/окна: пересчёт по релизу | 🔵 Низкий | ✅ сделано |
| 4.11 | Маркер сортировки на колонке | 🔵 Низкий | ✅ сделано |
| 4.12 | Конфиг-driven колонки + now-playing + dbl-click + scroll + repeat | 🔵 Низкий | ✅ сделано |
| 4.13 | Названия полей инфо-панели в конфиг | 🔵 Низкий | ✅ сделано |
| 5.1 | tracing вместо eprintln! | ⬜ желательно | ⬜ |
| 5.2 | builder для AppConfig | ⬜ желательно | ⬜ |
| 5.3 | Горячая перезагрузка конфигов | — отклонено | 🚫 |
| 6.1 | Каркас визуализации | 🔴 Высокий | ✅ сделано |
| 6.2 | Анализатор спектра | 🔴 Высокий | ✅ сделано |
| 6.3 | DsdDecoder + Ресемплеры | 🟡 Высокий | ✅ сделано |
| 6.4 | Осциллограмма | 🟡 Высокий | ✅ сделано |
| 6.5 | Спектрограмма | 🟡 Высокий | ✅ сделано |
| 6.6 | Переключение типа + UI настроек | 🟡 Высокий | ✅ сделано |
| 6.7 | Оптимизация визуализации + баги | 🔴 Высший | ✅ сделано |
| 6.8 | Bit-perfect и DSD native/DoP | 🔴 Высший | ⏳ Следующий ход |
| 6.9 | Полировка: seek, кэш, персистентность | 🟢 Средний | ⬜ Бэклог |
| 6.10 | Техническое документирование | 🔵 Низкий | ⬜ Бэклог |
